...
class DDFP(Base):
    ...
    def __init__(self, config):
        super(DDFP, self).__init__(config)
    ...
    def forward(self, state_and_pov):
        combined_featues = super(DDFP, self)._partial_forward(state_and_pov)
    ...
    def get_relevant_parameters(self):
        base_relevant_parameters = super(DDFP, self).get_relevant_parameters()
    ...
    def update_relevant_parameters(self, new_dict):
        super(DDFP, self).update_relevant_parameters(new_dict)
        self.epsilon = new_dict["epsilon"]
        self.clipped_reward = new_dict["clipped_reward"]


    def _discounted_measurements(self, states, last_state, goal_items, finished, forced_continued_digging):
        if finished:
            discounted_measurements_next_state = [0.] * self.goal_size
        else:
            predictions_tensor = self.forward([last_state])
            action_id = self.choose_best_action(predictions_tensor, last_state["goal"])
            discounted_measurements_next_state = predictions_tensor[0][action_id].detach().cpu().numpy()

            """
            if forced_continued_digging:
                # the next action will be digging (action 0)
                discounted_measurements_next_state = predictions_tensor[0][0].detach().cpu().numpy()
            else:
                action_id = self.choose_best_action(predictions_tensor, last_state["goal"])
                d_m_best_action = predictions_tensor[0][action_id]
                d_m_random_action = predictions_tensor[0].mean(dim=0)
                discounted_measurements_next_state = self.epsilon * d_m_random_action + (1 - self.epsilon) * d_m_best_action
                discounted_measurements_next_state = discounted_measurements_next_state.detach().cpu().numpy()
            """
    ...
    def calculate_gradients(self, state_storage: StateStorage, update_type="normal"):
        ...
        forced_continued_digging = state_storage.forced_continued_digging

        self.zero_grad(set_to_none=True)
        max_index = len(states)
        ...
        discounted_rewards = self._discounted_measurements(states, last_state, state_storage.goal_items, state_storage.finished, forced_continued_digging)
...
if __name__ == "__main__":
    ...
    DDFP_1 = DDFP(model_parameter_config)
    DDFP_2 = DDFP(model_parameter_config)
    test = sum([param.nelement() for param in DDFP_1.parameters()])
    print(test)
    start_time = time.time_ns()

    for param, new_param in zip(DDFP_1.parameters(), DDFP_2.parameters()):
        param.data = new_param.to("cpu")

    DDFP_2.update_relevant_parameters(DDFP_2.get_relevant_parameters())
