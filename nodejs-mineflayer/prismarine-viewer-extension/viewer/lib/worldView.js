...
class WorldView extends EventEmitter {
  ...
  listenToBot (bot) {
    ...
    this.listeners[bot.username] = {
      ...
      itemDrop: function (e) {
        const item = bot.entities[e.id].metadata[8]
        worldView.emitter.emit("entity", {id: e.id, name: e.name, pos: e.position, width: e.width, height: e.height, itemid:item.itemId,  itemcount:item.itemCount, nbtData:item.nbtData, username: e.username})
      },
      ...
      entitySpawn: function (e) {
        ...
        if (e.name === "item") return
        worldView.emitter.emit('entity', { id: e.id, name: e.name, pos: e.position, width: e.width, height: e.height, username: e.username, objectData:e.objectData })
      },
    ...
    for (const id in bot.entities) {
      ...
      if (e && e !== bot.entity) {
        if (e.name && e.name.toLowerCase() === "item") {
          const item = bot.entities[e.id].metadata[8]
