...
module.exports = (bot, { viewDistance = 6, IPC = false, output = 'output.mp4', frames = -1, width = 512, height = 512, milliseconds_between_frames=16, logFFMPEG = false, jpegOptions }) => {
  ...
  if (rtmpOutput) {
    ...
    if (IPC)  client.connect(host, () => { update()})
    else   client.connect(parseInt(port, 10), host, () => {update()})
  }
    ...
    if (rtmpOutput || ffmpegOutput) {
      ...
      imageStream.on('end', () => {
        ...
        if (idx < frames || frames < 0) {
          setTimeout(update, milliseconds_between_frames)
        } else {
